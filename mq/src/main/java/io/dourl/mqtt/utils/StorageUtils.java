...
public class StorageUtils {
    ...
    private static File getCacheDirectory(Context context, boolean preferExternal) {
        ...
        if (appCacheDir == null) {
            appCacheDir = context.getCacheDir();
            LoggerUtil.d("Can't define system cache directory! '%s' will be used.", "context.getCacheDir()");
        }
        return appCacheDir;
    }
    ...
    private static File getFilesDirectory(Context context, boolean preferExternal) {
        ...
        if (appFilesDir == null) {
            appFilesDir = context.getFilesDir();
            LoggerUtil.d("Can't define system cache directory! '%s' will be used.", "context.getFilesDir()");
        }
        return appFilesDir;
    }
    ...
    protected static boolean makeNoMedia(File cacheDir) {
        if (!cacheDir.exists()) {
            if (!cacheDir.mkdirs()) {
                LoggerUtil.d("Unable to create external cache directory");
                return true;
            }
            try {
                new File(cacheDir, ".nomedia").createNewFile();
            } catch (IOException e) {
                LoggerUtil.d("Can't create \".nomedia\" file in application external cache directory");
            }
        }
        return false;
    }
